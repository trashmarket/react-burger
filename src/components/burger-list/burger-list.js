...
function BurgerList({data}) {
    ...
    const [bun, setBun] = React.useState(data.filter(item => {
       return sort(item, 'bun') 
    })) 
    const [sauce, setSauce] = React.useState(data.filter(item => {
        return sort(item, 'sauce')
    }))
    const [main, setMain] = React.useState(data.filter(item => {
    ...
    return (
        <ul className={styles.list} ref={list}>
            <li >
                ...
                {
                    bun.map((item, index) => <BurgerCard index={index === 0 ? true: false} item={item} key={item._id}/>)
                }
                </div>
            </li>
            <li>
                ...
                <div className={styles.wrapper}>
                {
                    sauce.map((item, index) => <BurgerCard index={index === 2 ? true: false} item={item} key={item._id}/>)
                }
                </div>
            </li>
            <li>
            <h3 className="text text_type_main-medium mt-10 mb-6">Начинки</h3>
                <div className={styles.wrapper}>
                {
                    main.map((item, index) => <BurgerCard  item={item} key={item._id}/>)
                }
                </div>
            </li>
